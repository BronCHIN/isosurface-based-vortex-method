function [intMatrix, intSurface] = subourtine_SurfaceIntersection(surface1, surface2, varargin)
...
% *varargin 只有三个参数，debug，alogrithm('moller'和'rapid')及pointroundingtol，具体意义暂时未知

% OUTPUT:
%注意作者说，如果不输出相交边的话，计算效率会更快
%但是我们必须输出相交边，so不用考虑效率啦
% * intMatrix - sparse Matrix with n1 x n2 dimension where n1 and n2 are
%               number of faces in surfaces,Possible values: -2 (do not  know),  -1 (coplanar with unknown overlap), 0 (no intersections), 1 (intersects).
...
%
% ALGORITHM:
% Based on Triangle/triangle intersection test routine by Tomas M鰈ler, 1997.
...
if isa(surface1, 'triangulation') %判断obj是否为class_name类型。如果是,返回逻辑1(真);如果不是,返回逻辑0(假)。
  [surface1.faces, surface1.vertices] = freeBoundary(surface1);
elseif isa(surface1, 'delaunayTriangulation')  %在比较正式的 MATLAB 语言术语中，delaunayTriangulation 是 triangulation 的子类
  S = surface1;
  surface1 = [];
...
ok1 = isstruct(surface1) && isfield(surface1, 'vertices') && isfield(surface1, 'faces'); %isstruct - 确定输入是否为结构体数组
ok2 = isstruct(surface2) && isfield(surface2, 'vertices') && isfield(surface2, 'faces'); %isfield - 确定输入是否为结构体数组字段
...
while (k<=nVarargs)
  assert(ischar(varargin{k}), 'Incorrect input parameters') %assert - 条件为 false 时引发错误
...
% M鰈ler抯 method begins by checking the mutual intersection of each
% triangle with the plane of the other. To do so, it determines for each
% triangle on which side of the other triangle抯 supporting plane its
...
tMsk = (intMatrix==-2);
n = nnz(tMsk); %不为0的元素个数为 nnz(A)
if n>0
  [face1, face2] = find(tMsk); %[row,col] = find(___) 使用前面语法中的任何输入参数返回数组 X 中每个非零元素的行和列下标
